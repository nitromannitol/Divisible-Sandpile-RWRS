"""Check (or repair) the `rwrs.tex:<a>-<b>` citations inside the Lean sources.
...
def _paper_path() -> Path:
    """The paper: the copy pinned in this repository, or `$RWRS_PAPER`."""
    env = os.environ.get("RWRS_PAPER")
    return Path(env) if env else ROOT / "paper" / "rwrs.tex"
...
def main() -> int:
    ...
    for path in sorted((ROOT / "RWRS").rglob("*.lean")):
        ...
        block = header_block(text) if str(path).startswith(str(ROOT / "RWRS" / "Frozen")) else None
