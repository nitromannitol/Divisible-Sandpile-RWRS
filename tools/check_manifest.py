#!/usr/bin/env python3
"""Frozen-statement manifest checker for the RWRS formalization.
...
Verifies, against ledger/manifest.yaml:
  ...
  8. when a node has `closure_sha256`, its source-level RWRS dependency-closure
...
`RWRS/**/*.lean` outside `RWRS/Frozen/`.  It scans comment- and string-masked
...
manifest node protect those bytes, while this closure covers the unfrozen RWRS
support surface that D-015 targets.

This is conservative but not an elaboration-equivalent dependency oracle.
False positives can arise when a local binder shadows a uniquely named RWRS
...
can also make the heuristic declaration span incomplete.  Imported non-RWRS
...
class SourceDecl:
    """One explicitly named source declaration outside RWRS/Frozen/."""
...
def current_namespace(scopes: list[tuple[str, str]]) -> str:
    ...
    for kind, name in scopes:
        if kind != "namespace":
            continue
        if name.startswith("RWRS.") or name == "RWRS":
            parts = name.split(".")
        else:
...
def source_declarations() -> list[SourceDecl]:
    """Index explicit RWRS declarations outside the frozen tree."""
    ...
    for path in sorted(orrw_root.rglob("*.lean")):
        ...
        for line_no, line in enumerate(lines):
            ...
            if decl_match:
                ...
                if raw_name.startswith("RWRS.") or raw_name == "RWRS":
                    full_name = raw_name
                elif namespace:
                    full_name = f"{namespace}.{raw_name}"
                else:
                    full_name = raw_name
                if not full_name.startswith("RWRS."):
                    continue
...
class DeclarationResolver:
    ...
    def closure(self, source: str) -> set[SourceDecl]:
        """Return the transitive explicit RWRS declaration closure of source."""
...
def main() -> int:
    ...
    prod = [ROOT / "RWRS.lean"] + sorted((ROOT / "RWRS").rglob("*.lean"))
    for f in prod:
        if not f.exists():
            continue
    ...
    if args.emit:
        ...
        print(
            ...
            f"from {len(declarations)} indexed RWRS declarations")
        return 0
    ...
    print(
        ...
        f"{len(declarations)} RWRS declarations indexed)")
    return 0
