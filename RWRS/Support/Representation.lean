...
theorem odometer_isLUB (hG : G.Connected) (σ : V → ℝ) :
  ...
  | succ n ih =>
      intro x
      have hrec := RWRS.Frozen.recursion hG σ n x
      have hstep : ∀ y : V,
  ...
  | succ n ih =>
      ...
      · have hne : ∀ X : ℕ → V, X 0 = x → optimalStop G (excess σ) (n + 1) X ≠ 0 := by
        ...
        have hA : 0 ≤ walkOp G (odometer G σ n) x + scenery G σ x := by
          by_contra hc
          rw [not_le] at hc
          exact hz (by rw [value_eq hG, RWRS.Frozen.recursion hG σ n x, max_eq_right hc.le])
        rw [RWRS.Frozen.recursion hG σ n x, max_eq_left hA]
        ring
